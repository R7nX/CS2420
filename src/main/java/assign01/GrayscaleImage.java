...
/**
     ...
     *
     * @param other image to compare with this one
     * @return a boolean value representing if they are equal
     */
    @Override
    ...
    public GrayscaleImage normalized() {
        ...
        double currentBrightness = this.averageBrightness();

        double scaleFactor = 127.0 / currentBrightness;

        for (int row = 0; row < imageData.length; row++) {
            for (int col = 0; col < imageData[row].length; col++) {
                normalizedImage.imageData[row][col] = this.imageData[row][col] * scaleFactor;
            }
        }
    ...
    public GrayscaleImage mirrored() {
        ...
        double[][] mirrorImage = new double[rows][cols];

        for(int row = 0; row < rows; row++){
            for(int col = 0; col < cols; col++){
                mirrorImage[row][col] = imageData[row][cols - 1 - col];
            }
        }

        GrayscaleImage mirroredArray = new GrayscaleImage(mirrorImage);
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height) {
        if (startCol < 0 || startRow < 0 || width < 0 || height < 0 || startRow + height > imageData.length || startCol + width > imageData[0].length)
            throw new IllegalArgumentException("Invalid rectangle parameters. The specified rectangle is outside the bounds of the original image.");
        double croppedArray[][] = new double[width][height];
        GrayscaleImage croppedArrayObj = new GrayscaleImage(croppedArray);

        for (int row = 0; row < width; row++) {
            for (int col = 0; col < height; col++) {
                croppedArrayObj.imageData[row][col] = this.imageData[startRow + row][startCol + col];
            }
        }
        return croppedArrayObj;
    }
    ...
    public GrayscaleImage squarified() {
        // Finding the difference between the width and the height
        double[][] squareImage;
        ...
        squareImage = new double[min][min];

        int pixelToRemoveVertically = (height - min) / 2;
        int pixelToRemoveHorizontally = (width - min) / 2;

        for (int row = 0; row < min; row++) {
            for (int col = 0; col < min; col++) {
                squareImage[row][col] = imageData[row + pixelToRemoveHorizontally][col + pixelToRemoveVertically];
            }
        }

        GrayscaleImage squared = new GrayscaleImage(squareImage);
